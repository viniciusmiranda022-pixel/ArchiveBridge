...
/// <summary>
/// Projeção SQL (somente leitura) dos escopos com trabalho de EXPORTAÇÃO EV elegível — mesmo padrão de
/// <c>SqlEvDiscoveryPendingScopeReader</c> (Slice 3): usa a identidade de MANUTENÇÃO (aprovada
/// explicitamente por <c>EvWorkerBoundaryTests.MaintenanceIdentityIsRestrictedToApprovedCrossTenantInfrastructureOperations</c>),
/// é ESTRITAMENTE READ-ONLY (nenhum claim/UPDATE/INSERT/efeito de negócio) e devolve apenas
/// (tenant, projeto) DISTINTO — nunca connector, archive, política ou evidência.
/// </summary>
public sealed class SqlEvExportPendingScopeReader(TenantConnectionFactory connectionFactory, IClock clock) : IEvExportPendingScopeReader
{
    ...
    private const string SelectEligibleScopesSql =
        ...
        WHERE j.workload = @workload
          ...
          AND EXISTS (
              SELECT 1 FROM dbo.ev_export_requests er
              WHERE er.job_id = j.job_id AND er.tenant_id = j.tenant_id AND er.project_id = j.project_id)
