...
public interface IMappingStore
{
    ...
    /// <summary>
    /// Persiste atomicamente a nova versão: substitui a versão utilizável anterior (Superseded) e
    /// grava a nova versão e suas linhas de evidência. O índice único filtrado garante no máximo uma
    /// versão utilizável por onda (fail-closed).
    /// </summary>
    Task SaveAsync(TenantScope scope, MappingGenerationResult result, CancellationToken cancellationToken);
}
