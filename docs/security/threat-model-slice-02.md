...
| **Lease expirado persiste efeito** | dono grava com lease vencido (ainda não reaped) | a guarda exige `lease_expires_at_utc > @now`; heartbeat **PERIÓDICO** (`lease/3`) renova durante TODA a operação, e sua perda **cancela** a operação (nenhum efeito novo) ⇒ `Fenced` | `SqlJobLeaseManager.RenewAsync` (cercado); `PlanningHeartbeat.RunWhileAsync` |
...
| **I/O de filesystem sob lock SQL** | publicar o artefato dentro da transação retém locks de Job/tabela enquanto escreve em SMB/NAS (lento), bloqueando reaper e demais gravações | protocolo em DUAS transações curtas: **reserva** (tx1, sem I/O) → **publica FORA do SQL** → **finaliza** (tx2, valida o bundle ANTES de abrir a transação); nenhum `UPDLOCK/HOLDLOCK` durante `File.Write`/`Directory.Move`/leituras | status `PendingArtifact` (2); `UX_mcv_single_usable` (só `status=0`); `CK_mcv_status` 0..2 |
| **Queda entre reserva e publicação** | worker cai após reservar a versão e antes de publicar/finalizar ⇒ versão "presa" ou versão indevida no retry | reconciliação idempotente pela **impressão digital** (`GetPendingByFingerprintAsync`): a mesma geração recupera a reserva pendente e a finaliza — sem novo número de versão; a versão anterior **permanece utilizável** até a promoção | `generation_fingerprint` por versão; promoção `Pending→Usable` + `Supersede` na MESMA tx2 |
| **Worker defasado finaliza reserva** | worker com lease vencido promove a reserva de outra época | a finalização é **cercada** (mesma guarda `50030`, lease válido) e confere a reserva por versão+fingerprint+hash | guarda `50030` na tx2; `UPDLOCK/HOLDLOCK` na linha da versão |
| **Republicação valida só o CSV** | diretório final adulterado (manifesto/sha256/arquivo injetado) aceito por conferir só o hash do CSV | a republicação valida o **BUNDLE COMPLETO** (os três arquivos **e só eles**, hash do CSV, valor do sha256, manifesto vs. escopo/versão/tamanho/caminho); divergência recusada (imutável, fail-closed) | — (falha fechada; sem sobrescrita nem descarte do staging válido) |
| **Descarte fora da área de staging** | `DiscardAsync` recebe caminho arbitrário/symlink e apaga fora de `.staging` | descarte só de caminho ESTRITAMENTE sob `<root>/.staging` (contenção robusta + rejeição de reparse); recusa **antes** de qualquer `Directory.Delete` | — |
