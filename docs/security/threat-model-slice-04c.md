...
| Corrida de submissão de inventário (mesma versão calculada por dois envios concorrentes) | `UX_ev_cis_connector_version UNIQUE (connector_id, version)` é o backstop SQL; a segunda submissão converge para a linha já persistida pela primeira em vez de duplicar ou falhar. Comprovado por `ConcurrentAppendsOfTheSameConnectorVersionConvergeToOneRowWithoutDuplicating`. |
