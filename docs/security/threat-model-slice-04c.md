...
| Ameaça | Mitigação |
| --- | --- |
| Delta strategy implícita para versão/schema desconhecida | `EvDeltaStrategySelectionPolicy` nunca infere: versão não reconhecida por nenhuma família da matriz embarcada ⇒ `Unknown`; família reconhecida mas sem strategy elegível para a fase ⇒ `Unsupported`; empate de precedência entre elegíveis ⇒ `Ambiguous` — em NENHUM destes três casos o adapter EV é chamado. Comprovado por `UnknownOrUnrecognizedVersionSelectsNothing`, `TwoEligibleDescriptorsTiedAtTheHighestPrecedenceAreAmbiguousFailClosed`, `BaselineWithUnknownEvVersionBlocksFailClosedAndRecordsTheAttempt`. |
...
| Evidência de watermark persistida adulterada sendo lida como canônica | A persistência é fronteira NÃO CONFIÁVEL (mesmo princípio de `InventorySnapshot`/`EvExportManifest`): `EvWatermark.Rehydrate` recomputa `LineageHash` a partir dos campos REALMENTE carregados e recusa fail-closed (`EvWatermarkRejectedException(Tampered)`) qualquer divergência do hash persistido. Comprovado por `RehydrateFailsClosedWhenTheLineageHashDoesNotMatchTheLoadedFields`. |
