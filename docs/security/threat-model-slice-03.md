...
| --- | --- | --- | --- |
| **PowerShell injection** | valor de parâmetro carrega `;`/`\|`/`` ` ``/`$(` para injetar comando | executor CONTROLADO valida valores (recusa metacaracteres) e liga parâmetros TIPADOS (sem concatenação) | — |
| **Execução de script arbitrário** | chamador fornece script livre | a porta só aceita `CommandName` da allowlist + parâmetros tipados; nunca um script | — |
| **`Invoke-Expression`** | usar `iex` para avaliar string | recusado explicitamente ANTES de qualquer execução (além da allowlist) | — |
| **Command not on allowlist** | invocar cmdlet perigoso (`Remove-Item`) | allowlist explícita; fora dela ⇒ `EvPowerShellNotAllowedException` (o host nunca é chamado) | — |
| **Module/DLL/PATH hijacking** | carregar módulo/assembly malicioso do PATH | working directory controlado; execução não interativa; host isolado (Windows Worker) executa com privilégio mínimo | política de execution host explícita (produção) |
...
| **Credencial em linha de comando/log** | senha/token em parâmetro ou stdout | parâmetros nunca carregam segredo; saída SANITIZADA (redige `password=/token=/sas=`); nada de credencial persistida | — |
...
| **Output excessivo (DoS de memória)** | cmdlet devolve saída gigante | limite de tamanho de saída (truncamento sinalizado) | — |
| **Timeout / cmdlet travado** | sonda pendura o worker | timeout OBRIGATÓRIO por requisição (CTS); estouro ⇒ `TimedOut` fail-closed | heartbeat periódico do Job |
| **Saída malformada** | JSON inválido para forjar capacidade | parser fail-closed ⇒ achado `EV_DISCOVERY_OUTPUT_INVALID` (avaliação Failed) | — |
...
| **Queda entre fases** | worker cai após reservar/publicar | reconciliação idempotente pelos hashes (config+evidência); recupera a reserva sem versão indevida | `IX_evd_hashes` |
...
tenant. A saída do executor é redigida (`[REDACTED]`) e limitada.
