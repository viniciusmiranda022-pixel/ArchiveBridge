...
O gate exige **relatório de validação do Purview em tenant controlado**. O **protocolo de validação** está em [`evidence/0006-plano-validacao-purview.md`](evidence/0006-plano-validacao-purview.md) e separa **dois gates**: **Gate A — evidência pré-código** (executável manualmente em tenant; **bloqueia a aceitação** do ADR) e **Gate B — contrato de implementação** (dependente de código; obrigatório **antes de produção**, mas **não** bloqueia a aceitação). **Não se exige código inexistente como evidência prévia**, e **não** se executa carga real acima de 100 GB nesta fase. A **execução do Gate A é externa e permanece pendente** (Evidence Owner e responsável técnico pelo tenant a atribuir). Este ADR permanece **`proposto`** até o Gate A estar anexado e revisado e a **aceitação formal do Decision Owner** (Vinicius Miranda) ser registrada.
