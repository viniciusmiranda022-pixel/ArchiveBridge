...
| `SqlPartitionExecutionStoreLatency` | `Measured` (mesma condição `ARCHIVEBRIDGE_TEST_SQL` acima) | `ObservedMetric` por execução do harness, sobre `SaveAsync` com fixture real (custódia → inspeção → plano, persistidos via os mesmos casos de uso da Slice 4B) | `PartitionExecutionStoreBenchmarkTests` (AB-I7-004) — ver performance-baseline-report-i7.md §3 |
| `SqlPurviewMappingCsvStoreLatency` | `Measured` (mesma condição acima) | `ObservedMetric` por execução do harness, sobre `GeneratePurviewMappingCsvUseCase.ExecuteAsync` (Reserve+Finalize reais) com fixture de onda plenamente verificada | `PurviewMappingCsvStoreBenchmarkTests` (AB-I7-004) |
| `ReconciliationCertificateLatency` | `Measured` (mesma condição acima) | `ObservedMetric` por execução do harness, sobre `IssueReconciliationCertificateUseCase.ExecuteAsync` (exercita `IssueOrConvergeAsync` real) com fixture de onda plenamente reconciliada (`happy PASS`) | `ReconciliationCertificateStoreBenchmarkTests` (AB-I7-004) |
